...
async def zombies_handler(client, message):
    ...
    if app.long() != 2:
        ...
        if temp_count > 0:
            await message.reply(f"Terdeteksi {temp_count} Musuhnya Bapak Naruto\nGunakan {app.Trigger[0]}tsukoyomi untuk melenyapkannya dari bumi.", quote=True)
        else:
            await message.reply("KGA ADA MUSUH YANG TERDETEKSI EMHH.\nGroup bersih nan sentosa ! 😃", quote=True)

    elif app.long() == 2 and message.command[1] == "kyaa":
        await message.reply("*SHINRA TENSEI* . . .", quote=True)
        ...
        async for x in app.get_chat_members(chat_id=message.chat.id):
            if x.user.is_deleted:
                ...
                try:
                    ...
                    await app.error(e)
        await message.reply(f"WATASHIWA DONE\n\n*Total:* {count+admin_count}\n*Removed:* {count}\n*Not Removed:* {admin_count}\n\n*Note:* Not removed accounts can be admins or the owner", quote=True)

    elif app.long() == 2 and message.command[1] != "kyaa":
        await message.reply(f"Check {app.Trigger[0]}help tsukoyomi` to see how it works !", quote=True)
    else:
        await message.reply("Something went wrong, please try again later !", quote=True)